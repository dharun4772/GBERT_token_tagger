    ...
    def forward(self, input):
        if not self.adversarial_mode:
            ...
            if self.delta is None:
                self.update_delta(required_grad=True)
            return self.pertubated_input
    
    def update_delta(self, required_grad=True):
        if not self.adversarial_mode:
            return True
        if self.delta is None:
            delta = torch.clamp(
        ...
        self.delta = delta.float().detach().requires_grad_(required_grad)
        self.pertubated_input = (self.input.to(delta).detach() + self.delta).to(
            self.input
        )
        return True
...
def hook_sift_layer(
    ...
    """
    Hook the sift perturbation layer to an existing model. With this method, you can apply adversarial training
